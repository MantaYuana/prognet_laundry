...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ $isCustomer ? route('customer.orders.index') : route('outlet.customer.order.index', ['outlet' => request()->route('outlet'), 'customer' => request()->route('customer')]) }}" 
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ $isCustomer ? route('customer.orders.store') : route('outlet.customer.order.store', ['outlet' => request()->route('outlet'), 'customer' => request()->route('customer')]) }}" 
                  method="POST" 
                  id="orderForm">
                @csrf
                ...
                <div class="card bg-base-100 shadow-xl">
                    <div class="card-body">
                        ...
                        <div class="card-actions flex items-center gap-4 justify-end mt-6">
                            <button type="button" 
                                    onclick="window.location='{{ $isCustomer ? route('customer.order.index') : route('outlet.customer.order.index', ['outlet' => request()->route('outlet'), 'customer' => request()->route('customer')]) }}'" 
                                    class="btn btn-ghost">
                                Cancel
                            </button>
    ...
    <script>
        ...
        @endphp
        const validatePromoUrl = "{{ $isCustomerUser ? route('customer.order.validate-promo', request()->route('outlet')) : route('outlet.staff.order.validate-promo', ['outlet' => request()->route('outlet')]) }}";
