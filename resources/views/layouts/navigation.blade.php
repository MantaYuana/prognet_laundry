    ...
    <div class="flex flex-col drawer-content">
        ...
        <main class="flex-1 overflow-y-auto bg-base-200">
            ...
            @php
                ...
                } elseif (request()->routeIs('customer.order.show')) {
        ...
        <aside id="sidebar" class="flex flex-col w-64 min-h-screen text-white transition-all duration-300 bg-gradient-to-b from-teal-600 via-teal-700 to-teal-800">
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                <a href="{{ route('customer.order.index') }}" 
                   class="flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all duration-300 {{ request()->routeIs('customer.order.*') ? 'bg-gradient-to-r from-teal-500 to-cyan-500 shadow-lg' : 'hover:bg-teal-600/50' }}">
                    <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('customer.order.*') ? 'text-white' : 'text-teal-200' }}" 
                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                ...
                class="flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all duration-300 {{ request()->routeIs('outlet.index') || (request()->routeIs('outlet.*') && !request()->routeIs('outlet.services.*') && !request()->routeIs('outlet.promo.*')) ? 'bg-gradient-to-r from-teal-500 to-cyan-500 shadow-lg' : 'hover:bg-teal-600/50' }}">
                    
                    <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('outlet.index') || (request()->routeIs('outlet.*') && !request()->routeIs('outlet.services.*') && !request()->routeIs('outlet.promo.*')) ? 'text-white' : 'text-teal-200' }}" 
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                            d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                    </svg>
                    <span class="font-medium sidebar-text {{ request()->routeIs('outlet.index') || (request()->routeIs('outlet.*') && !request()->routeIs('outlet.services.*') && !request()->routeIs('outlet.promo.*')) ? 'text-white' : 'text-teal-100' }}">
                        Outlet
                    </span>
                </a>
                ...
                @if($currentOutlet)
                    ...
                    <a href="{{ route('outlet.promo.index', ['outlet' => is_object($currentOutlet) ? $currentOutlet->id : $currentOutlet]) }}" 
                       class="flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-all duration-300 {{ request()->routeIs('outlet.promo.*') ? 'bg-gradient-to-r from-teal-500 to-cyan-500 shadow-lg' : 'hover:bg-teal-600/50' }}">
                        <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('outlet.promo.*') ? 'text-white' : 'text-teal-200' }}" 
                             fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                                  d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                        </svg>
                        <span class="font-medium sidebar-text {{ request()->routeIs('outlet.promo.*') ? 'text-white' : 'text-teal-100' }}">
                            Promo
                        </span>
                    </a>
                @endif
